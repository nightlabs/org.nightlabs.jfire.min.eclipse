...
public class CommentDataBlockEditor extends AbstractDataBlockEditor
{
	...
	public static class Factory extends AbstractDataBlockEditorFactory {
		...
		public DataBlockEditor createDataBlockEditor(IStruct struct, DataBlock dataBlock) {
			return new CommentDataBlockEditor(struct, dataBlock);
		}
	}

	private static class CommentDataBlockEditorComposite 
	extends 
	AbstractDataBlockEditorComposite
	implements
	ModifyListener,
...
//		private Composite wrapper;
		...
		public CommentDataBlockEditorComposite(DataBlockEditor blockEditor, Composite parent, int style) {
			...
			refresh(getStruct(), getDataBlock());
		}
		...
		public void doRefresh() {
			try {
				commentData = (TextDataField) getDataBlock().getDataField(PersonStruct.COMMENT_COMMENT);
				refresh();
...
//				labelTitle.setLayoutData(new GridData(GridData.FILL_HORIZONTAL));
		...
		public DataFieldEditor getNewEditorInstance(IStruct struct, DataField data) {
			return new CommentDataBlockEditorComposite(getDataBlockEditor(), getParent(), getStyle());
		}
	...
	protected CommentDataBlockEditor(IStruct struct, DataBlock dataBlock) {
		super();
	}

	@Override
	protected IDataBlockEditorComposite createEditorComposite(Composite parent) {
		return new CommentDataBlockEditorComposite(this, parent, SWT.NONE);
	}
}
