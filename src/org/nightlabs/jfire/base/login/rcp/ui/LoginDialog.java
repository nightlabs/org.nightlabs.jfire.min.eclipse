...
public class LoginDialog extends TitleAreaDialog
{
	...
	public LoginDialog(Shell parent)
	{
		...
		setTitleImage(SharedImages.getSharedImageDescriptor(JFireRCPLoginPlugin.getDefault(), LoginDialog.class, null, ImageDimension._75x70).createImage());
		try {
		...
		try {
			...
			if (async) {
				Job job = new Job(Messages.getString("org.nightlabs.jfire.base.ui.login.LoginDialog.authentication")) { //$NON-NLS-1$
					...
					{
						doCheckLogin(saveSettings, monitor, loginStateListener, LoginDialog.this.getShell().getDisplay());
						return Status.OK_STATUS;
					}
				};
				job.schedule();
...
//				hadError = false;
//				return
				doCheckLogin(saveSettings, monitor, loginStateListener, Display.getDefault());
			}
	 ...
	 * @return
	 */
	private void doCheckLogin(boolean saveSettings, IProgressMonitor monitor, final LoginStateListener loginStateListener, Display display)
	{
