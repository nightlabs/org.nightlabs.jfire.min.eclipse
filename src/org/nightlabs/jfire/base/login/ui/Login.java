...
//implements InitialContextProvider
{
	...
	private static final String LOGIN_STATE_LISTENER_PRIORITY = "priority"; //$NON-NLS-1$
	...
	private static ISingletonProvider<Login> sharedInstanceLogin = SingletonProviderFactory.createProvider();
	...
	static {
		sharedInstanceLogin.setFactory(new ISingletonFactory<Login>() {
			@Override
			public Login makeInstance() {
	...
	public static boolean isLoggedIn() {
		LoginState ls = Login.sharedInstanceLogin.getInstance().currentLoginState;
		...
		public void run() {
			...
			try {
				...
				try{
					...
					lHandler.handleLogin(handlingLoginData, Login.sharedInstanceLogin.getInstance().getRuntimeConfigModule(), loginResult);
	...
	private void handleSuccessfulLogin() {
		...
		needRestart = getLoginHandler().needsRestartAfterSuccessfullLogin();
	...
	protected org.nightlabs.jfire.base.jdo.JDOObjectID2PCClassNotificationInterceptor getObjectID2PCClassNotificationInterceptor() {
		return objectID2PCClassNotificationInterceptor;
	}
	
	protected void setObjectID2PCClassNotificationInterceptor(org.nightlabs.jfire.base.jdo.JDOObjectID2PCClassNotificationInterceptor objectID2PCClassNotificationInterceptor) {
		this.objectID2PCClassNotificationInterceptor = objectID2PCClassNotificationInterceptor;
	}
	...
	{
		return Login.sharedInstanceLogin.getInstance();
	}
	...
	{
		if (doLogin) {
			Login.sharedInstanceLogin.getInstance().doLogin();
		}
...
//			if (loginData != null)
	...
	private static class LoginStateListenerRegistryItemSorter
	implements Comparator<LoginStateListenerRegistryItem>
	{
		@Override
		public int compare(LoginStateListenerRegistryItem l0,
				LoginStateListenerRegistryItem l1) 
		{
			return l0.getPriority() - l1.getPriority();
		}
	}
	
	public static final int DEFAULT_LOGIN_STATE_PRIORITY = 100;
	...
	public void addLoginStateListener(LoginStateListener loginStateListener, IAction action) {
		addLoginStateListener(loginStateListener, action, DEFAULT_LOGIN_STATE_PRIORITY);
	}

	public void addLoginStateListener(LoginStateListener loginStateListener, IAction action, int priority) {
	...
	protected void changeLoginStateAndNotifyListeners(final LoginState newLoginState) {
		...
		synchronized (loginStateListenerRegistry) {
			try {
				...
				// This is now done in high priority JDOObjectID2PCClassNotificationInterceptorLoginStateListener
...
//					} catch (Throwable t) {
//						Login.logger.error("Cache could not be opened!", t); //$NON-NLS-1$
//					}
//
...
//				}
		...
		// Sort LoginStateListeners by priority before notification
		Collections.sort(loginStateListenerRegistryItems, new LoginStateListenerRegistryItemSorter());
	...
	public void processElement(IExtension extension, IConfigurationElement element) throws Exception
	{
		if (Login.LOGIN_STATE_LISTENER_ELEMENT.equals(element.getName())) 
		{
			...
			String attributePriority = element.getAttribute(Login.LOGIN_STATE_LISTENER_PRIORITY);
			int priority = DEFAULT_LOGIN_STATE_PRIORITY;
			if (attributePriority != null) {
				try {
					priority = Integer.parseInt(attributePriority);	
				} catch (NumberFormatException e) {
					// do nothing but leave default priority
				}
			}
			addLoginStateListener(listener, null, priority);
		}
	}

	public static Login.AsyncLoginResult testLogin(LoginData _loginData) {
		return Login.sharedInstance().getLoginHandler().testLogin(_loginData);
	}
...
//			Login.logger.debug(JFireLoginPlugin.class.getClassLoader());
